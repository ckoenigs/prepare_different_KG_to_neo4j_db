...
g = driver.session(database='rtxkg2')
...
def prepare_node_infos():
    ...
    for label, in g.run(query):
        ...
        list_of_dict.append({'id': identifier, 'name': label, 'category':identifier})
        node_label_to_id[label] = identifier
        list_legend.append(f'{identifier}')
        identifier += 1
        ...
        for labels_1, labels_2, in g.run(query):
            ...
            for label_1 in labels_1:
                for label_2 in labels_2:
                    if  (label_1, label_2) in set_tuple or  (label_2, label_1) in set_node_labels_1:
                        continue
                    set_tuple.add((label_1, label_2))
                    list_dict_edges.append(
                        {'source': node_label_to_id[label_1], 'target': node_label_to_id[label_2], 'id': counter})
                    counter += 1
...
def main():
    ...
    # with open('analysis/node.json', 'w', encoding='utf-8') as f:
    #     f.write(json.dumps(list_of_dict))
    # with open('analysis/edges.json', 'w', encoding='utf-8') as f:
    #     f.write(json.dumps(list_dict_edges))
